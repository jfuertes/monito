...
var Facebook_correctLogin	= function () {
	...
	FB.api('/me', function(response) {
		...
		if (document.getElementById('fbStatus')){
			//si ya esta logeado y ese login concuerda con base de datos
			if(facebookUser.id!=null){
					$.ajax({
					  method: "POST",
					  url: "api/validatealufb.php",
					    ...
					    if(msg=="correcto"){
							...
							$("#entraalufb").click(function() {
								//alert("ola");
      								$.ajax({ 
									  method: "POST",
									  url: "api/logeofbalucorrecto.php",
									  data: { 'username': facebookUser.id}
										})
									  .done(function( msg ) {
									    if(msg=="correcto"){
							...
							$("#entraalufb").click(function() {
								//alert("ola");
      								$.ajax({ 
									  method: "POST",
									  url: "api/crearcuentaaluconfb.php",
									  data: { 'username': facebookUser.id}
										})
									  .done(function( msg ) {
									    if(msg=="correcto"){
