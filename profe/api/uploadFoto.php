...
if($file && move_uploaded_file($_FILES["file"]["tmp_name"], "../../IMG/profes/".$file))
{
	echo $file;
}
...
if(isset($r[0]["extension_img"])){
    ...
    unlink("../../IMG/profes/".$username.".".$r[0]["extension_img"]);
}  

}
              

rename("../../IMG/profes/".$file, "../../IMG/profes/".$username.".".$extension);


$q = "UPDATE  mp_profesor set extension_img=:extension_img where username=:username";
                            $stmt = $dbh->prepare($q);
                            $stmt->bindParam(':username', $username, PDO::PARAM_INT);
